...
以下是所有需要下载的图片URL及其对应的本地文件名：

## 图片映射

1. **hero-background.jpg** - 首页Hero背景图
...
3. **logistics-solution.jpg** - 物流解决方案图
...
5. **petrochemical-case.jpg** - 石油化工案例
...
请将以上URL的图片下载到 `public/images/` 目录，文件名与上述映射保持一致。

可以通过以下方式下载：
1. 使用浏览器直接访问URL并保存
2. 使用下载工具（如wget、curl）
3. 运行 `scripts/download-assets.js` 脚本（需要网络环境允许访问Google）
